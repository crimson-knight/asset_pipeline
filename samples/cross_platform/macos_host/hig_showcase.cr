...
def wrap_in_scene(slug : String, focal : UI::View) : UI::View
  ...
  when "gallery"
    fp = case slug
         when "image-views", "collections" then :grid_full
...
def build_component(slug : String) : UI::View
  ...
  when "collections"
    ...
    # HIG illustration shows a 4-column image grid. We render a 3-column
    # photo-tile grid using UI::ListView in grid mode (layout: :grid,
    # columns: 3). Each tile is a VStack with a placeholder image label
  ...
  when "progress-indicators"
    ...
    spinner_row = UI::HStack.new(spacing: 24.0)
    ...
    bar_hdr = UI::Label.new("Linear progress (determinate)")
    ...
    indet_hdr = UI::Label.new("Linear progress (indeterminate)")
    ...
    upload_hdr = UI::Label.new("Determinate with cancel")
