...
| 3 | SwiftUI Native Bridge | **Dispatch A stopped early (2026-05-20)** — awaiting architect decision on Phase 1 cascade-proof mechanism | Foundation: `7756d25 6996ba7`; Dispatch A: `7486040 ed51fd7 dfe1274 bdfcf18` (Button migration both renderers + tint cascade + sample builds wired + helper extraction) | — | Owner chose Option B (SwiftUI Default Supremacy). Button migration is clean and complete: ~460 lines of brand customization deleted across both renderers; 41 specs passing; Crystal→C→Swift tint cascade verified working via NSLog. But the macOS brand-cascade pixel-pivot proof from Phase 1 (#19) no longer works under SwiftUI: `.borderedProminent` button in an offscreen non-key window renders in inactive-appearance mode (washed-out grey, ΔE=0 between magenta and green brand runs). NOT a Phase 3 defect — it's a Phase 1 validation-infrastructure question. Implementer recommends swift-snapshot-testing (Phase 3 roadmap Step 8–9) for future cascade proofs. See handoff for the three resolution options. Dispatch B (34 remaining widgets) is gated on this decision |
