...
| 3 | SwiftUI Native Bridge | Implementer Active (2026-05-20) | — | — | High-risk; budget remediation loops. Branch `phase-03-swiftui-native-bridge` from `feature/utility-first-css-asset-pipeline@5628023`. **Scope addition:** also repair `src/ui/native/objc_collections.cr` link gap so `crystal spec` from repo root works (per owner decision) |
