...
| 6 | Side-by-Side Demo App | Not Started — depends on 6.5 | — | — | Per `handoff/planning-retrospective-2026-05-22.md` Principle 6 + Phase 6.5 insertion, Phase 6 development uses the audit harness Phase 6.5 ships. |
| **6.5** | **Audit-Infrastructure-First** (added 2026-05-22 per retrospective) | Not Started — runs BEFORE Phase 6 | — | — | NEW phase resolving the Phase-7-depends-on-Phase-6 audit-first conflict. Ships reusable harness (visual-diff baselining, CDP harness extensions, AXTest/XCUITest patterns, accessibility audit drivers) that Phase 6 uses during dev and Phase 7 wraps into CI. Folder created at `phases/phase-06.5-audit-infrastructure-first/` (README only; implementation.md + validation.md pending brief authoring against `schemas/phase_brief.schema.json`). |
| 7 | CI Integration for Accessibility & Visual Verification | Not Started — depends on 6.5 + 6 | — | — | Re-scoped 2026-05-22: was "build audit infrastructure" (now in 6.5); rescoped to "integrate Phase 6.5's harness into GitHub Actions CI; run on every PR." |
