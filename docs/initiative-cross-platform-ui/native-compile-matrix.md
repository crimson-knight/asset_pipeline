...
| macOS (`acrystal -Dmacos`) | `attempted-blocked` (spec-entry require missing) | Spec entry files must `require "../../src/ui"` before `require "../../src/ui/ax_test"` so the `crystal_ui_*_callback_dispatch` Crystal `fun` defs are linked. Fix is one-line per AX spec; resolved during the Deliverable 2 batch moves for the 14 `spec/native_macos/` specs. |
