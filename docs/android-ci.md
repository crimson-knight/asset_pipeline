...
- The mandatory native class list tests real controls, callbacks, navigation,
  ...
  picker, async images, button reads, host settings, text field styles, the host appearance, and platform storage, secrets and files. It verifies completed nonempty instrumentation plus clean
...
each runtime API in `[31, 35, 36]` (the floor, an intermediate release and
the target), with an x86_64 Google APIs Pixel 6 emulator; the newest released
runtime (`ANDROID_NEXT_RUNTIME`, Android 17) has its own nightly lane,
`android-next.yml`, outside the pull-request gate.
...
run every matrix entry even if one fails. They trigger on relevant pull-request
changes, relevant pushes to `main`, manual dispatch, and a nightly schedule
(see below). The workflow's permissions are `contents: read`; only the report
job adds `issues: write`; there is no release, publishing, signing-secret or
deployment step. Superseded runs of the same ref are cancelled. Repository branch
...
## Continuous runs and the failure issue (2026-09-10)

The workflow also runs on a nightly schedule (`17 6 * * *` UTC), so a change
in the runner image, an SDK package or the emulator is found the morning
after it lands rather than on the next pull request. GitHub fires scheduled
workflows from a repository's **default branch** only; on
`crimson-knight/asset_pipeline` the default is still `phase-10-c-0`, which
carries no Android workflow, so the schedule is armed but dormant until the
Android branch becomes the default. Until then `workflow_dispatch` on
`android-target` is the continuous run.

A `report` job runs after the matrix on every run that is not a pull request
(pull requests carry their own checks) and hands the matrix result to
`scripts/ci/report_outcome.sh`:

- **failure**, no open issue for the lane: open one, labeled `ci-failure` and
  `lane:android-native`, mentioning and assigning the maintainers (the
  repository variable `CI_MAINTAINERS`, handles separated by spaces or
  commas; the repository owner when it is unset);
- **failure**, an open issue: comment on it, so a persisting failure is one
  thread and not one issue a night;
- **success**, an open issue: comment that the lane recovered and close it;
- **cancelled** or skipped: nothing, a cancelled run is not a verdict.

The script exits nonzero only when it cannot report; the red stays on the
gate job. `scripts/tests/report_outcome.sh` is its falsification harness: a
stub `gh` records every call and nine scenarios assert exactly what the
script must and must not do (create, comment, close, the default and explicit
maintainers, the assignee fallback, the dry run). The job's permissions are
`contents: read` and `issues: write`; the workflow keeps `contents: read`.
A flaky lane opens and closes issues; the answer to that is the one this
lane has always used, name the flake and fix it (the run table above).

The live proof of the reporter, without a failing gate, is the dispatch
input `report_selftest`: the report job then opens a `lane:selftest` issue
that tags the maintainers and closes it again, with the runner's own token:

```bash
gh workflow run android-native.yml -R crimson-knight/asset_pipeline \\
  --ref android-target -f report_selftest=true
```

API levels are strings in the matrix because Android names minor SDK
releases (`36.1`, `37.0`, `37.2`). The newest released runtime, `37.0`
(Android 17), lives in `android-next.yml` (nightly and dispatch, its own
`lane:android-next` issue) until it is stable enough to gate pull requests;
see the Android 17 section below. The compile and target SDK stay at 36.

`docs/support-matrix.md` is generated by `ruby scripts/support_matrix.rb`
from `config/android_toolchain.env`, this workflow, the Apple pins and
`config/support_proof.yml` (what has actually run, dated). The host-contracts
...
The third run with the lane's own launcher (34482885314, 13dffefb) got the
Android 17 image past every earlier wall: boot in 51 s, the input service on
the first attempt, the native build, both APK installs on the 4 GiB data
partition, and the host contracts. The instrumentation then aborted inside
the first device test with `INSTRUMENTATION_ABORTED: System has crashed.`;
the system log shows the launcher's input-dispatch ANR, the WebView loader
dying and an activity dump timing out, which is the image's system server
going down, not the app. Three runs, three different failures, all in the
image's own machinery: on this runner the `google_apis` x86_64 Android 17
image is not yet stable under SwiftShader. The newest released runtime
therefore has its own lane, `android-next.yml`: nightly and on dispatch, the
same gate on the same launcher, reporting to `lane:android-next`, and never
part of the pull-request check, so an unstable new image cannot mask a real
regression on the supported runtimes. `ANDROID_NEXT_RUNTIME` in the
toolchain env names it; the matrix takes it when its lane has been green
long enough (the policy's first step for a new release).
