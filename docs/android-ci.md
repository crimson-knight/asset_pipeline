...
each runtime API in `[31, 35, 36, 37.0]` (the floor, an intermediate release,
the target, and the newest released runtime, Android 17), with an x86_64
Google APIs Pixel 6 emulator.
...
The matrix now carries `37.0`, Android 17, the newest released runtime. The
compile and target SDK stay at 36; a runtime above the compile floor is
exactly what the nightly is for. API levels are strings in the matrix
because Android names minor SDK releases (`36.1`, `37.0`, `37.2`) and the
