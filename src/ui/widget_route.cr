# Phase 10B.0 — UI::WidgetRoute resolver entry point.
#
# `UI::WidgetRoute.resolve(intent_id, context)` is the public API screens
...
# raises `UI::WidgetRoute::UnresolvableDefault` if neither an override nor a
# default is registered.
#
# Example call-site (proven to compile against `UI::SwipeActionRow`):
#
#     row_class = UI::WidgetRoute.resolve(:swipe_actions, context)
...
# must either install a `UI::App.override_widget` for the platform or
...
# `register_*_override` time inside `UI::WidgetRoute::Registry`. This keeps
...
module UI
  module WidgetRoute
    ...
    # `UI::WidgetRoute::UnresolvableDefault` if no widget is registered.
    #
    ...
    def self.resolve(
      ...
      widget = UI::WidgetRoute::Registry.resolve_for(intent_id, context)
      if widget
      ...
      raise UnresolvableDefault.new(
        ...
        "Either install a default via `UI::WidgetRoute::Registry.register_default(#{intent_id.inspect}, #{context.platform.inspect}, ...)` " \
        "OR register an override via `UI::App.override_widget(#{intent_id.inspect}, MyWidget)`. " \
    ...
    #
    # NOTE: Class C system-action dispatch lives in `UI::SystemAction.perform`
    # at `src/ui/system_action.cr`. It used to share this file under the
    # old `UI::Intent.dispatch` name; the 10D-final rename split the two
    # responsibilities so the routes-vs-actions semantics is clear.
    ...
    # `UI::WidgetRoute::Registry.platform_supported?` so a declaration like
    ...
    private def self.first_missing_capability(
      ...
      declared = UI::WidgetRoute::Registry.declared_capabilities_for(widget_class, intent_id) ||
                 ({} of Symbol => UI::WidgetRoute::Registry::CapabilityValue)
      required.each do |key, needed|
        next unless needed
        value = declared[key]?
        return key unless UI::WidgetRoute::Registry.platform_supported?(value, platform)
      end
      nil
    end
  end
