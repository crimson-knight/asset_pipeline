...
{% if flag?(:android) %}
  ...
  module UI::Android
    ...
    class Renderer < UI::PlatformVisitor
      ...
      # the `:swipe_actions` intent.
      #
      # # Why this is a LinearLayout, not M3 SwipeToDismissBox
      #
      # The honest Android-idiomatic mapping for swipe-revealed row
      # actions is `androidx.compose.material3.SwipeToDismissBox`. The
      # current `src/ui/native/android_bridge.c` is a View-system bridge
      # (`android_view_new`, `android_view_new_themed`, MaterialButton,
      # MaterialCardView, etc.) with NO Jetpack Compose interop. Routing
      # through `SwipeToDismissBox` requires a Compose-host JNI surface
      # (`ComposeView.setContent { ... }`) that does not exist today.
      #
      # Until that bridge lands, this visit falls back to the same shape
      # as `visit(InlineActionRow)`: a horizontal `LinearLayout` of
      # (leading actions, content, trailing actions) using
      # `MaterialButton` siblings dispatched through `UI::Button`. This
      # matches the `:partial` `supports_role_destructive` declaration on
      # `UI::AndroidSwipeActionRow` — destructive role tints the button,
      # but the swipe-reveal + confirm gesture is absent.
      #
      # # When the Compose bridge lands
      #
      # Replace the body with a `ComposeView` + `setContent { SwipeToDismissBox(...) { ... } }`
      # call sequence and flip
      # `UI::AndroidSwipeActionRow#declares_capabilities` to
      # `supports_role_destructive: true`. See
      # `docs/initiative-cross-platform-ui/handoff/phase-10-b-1c-close.md`
      ...
      def visit(view : UI::AndroidSwipeActionRow)
        ...
        # Center children vertically along the row baseline.
