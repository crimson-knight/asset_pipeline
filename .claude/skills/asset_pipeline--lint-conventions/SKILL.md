...
Will ship after Phase 10C.0 lands the `spec/web/` + `spec/native_*/`
split.

### Family 3 — architectural (deferred to 10A.0c)
Narrow `build`-method-scoped rules; require false-positive fixtures
before widening scope.

### Family 4 — test_id hygiene (deferred to 10A.final)

### Family 5 — directory + multi-target
The directory rule ships in 10C.0; deep rules in 10A.final.

## Adding a new rule

1. Create the rule file under
   `src/lsp_rules/family_<N>_<topic>/<name>_rule.cr`.
2. Subclass `ConventionRule` and implement `rule_name` +
   `check(file_path, content) : Array(Diagnostic)`.
3. Register the new rule in `scripts/lint_conventions.cr`
   `load_rules` (and `require` it at the top of the runner).
...
Add the lint step to `.github/workflows/<workflow>.yml`:
...
Phase 10C.0 wires the root `Makefile` `make lint` target.

## Pre-commit hook

Optional — drop in `.git/hooks/pre-commit`:
...
This skill ships in `asset_pipeline`'s `.claude/skills/`. Per the
shards (not `shards-alpha`) AI-docs auto-detection rule, consumers
who add `asset_pipeline` as a dependency receive the skill installed
under their `.claude/skills/asset_pipeline--<skill_name>/` namespace.
Because this skill's source directory is already named with the
`asset_pipeline--` prefix (matching the brief's specified name),
shards will install it under a doubly-prefixed path. See the
`phase-10-a-0a-close.md` handoff for the audit details.

## Why a Crystal runner instead of AmberLSP?

AmberLSP's `CustomRule` mechanism is YAML-regex-only and has no
Crystal compiled rule loader. Per the Phase 10 architecture decision
